      ...
      <div class="cart-body">
        ...
        v-for="(cart, index) in cartInfoList" :key="cart.connect_goods_se_sku_id">
          ...
          <li class="cart-list-con5">
            ...
            <input autocomplete="off" type="text" minnum="1" class="itxt" :value="cart.purchase_num"
              @change="handler('change', $event.target.value * 1, cart)">
            <a href="javascript:void(0)" class="plus" @click="handler('add', +1, cart)">+</a>
          </li>
          <li class="cart-list-con6">
            <span class="sum">{{ cart.payment }}</span>
          </li>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function(type, disNum, cart){
      // 在订单已经纯在的情况下，应付三种不同的情况：
      // 变化量（1）  变化量（-1） input最终的个数（非变化量）
      switch (type) {
        case "add":
          disNum = 1;
          break;
        case "minus":
          disNum = cart.purchase_num > 1 ? -1 : 0;
          break;
        case "change":
          if (isNaN(disNum) || disNum < 1) {
            disNum = 0;
          } else {
            disNum = parseInt(disNum) - cart.purchase_num;
          }
      }
      ...
      try {
        await // 派发action
          this.$store.dispatch("addOrUpdateShopCart", {
            skuId: cart.connect_goods_se_id,
            skuNum: disNum
          });
        // 再次获取服务器最新的数据进行展示
        this.getData();
