  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(address, index) in addressInfo" :key="address.id">
        <span class="username" :class="{ selected: address.isDefault == 1 }">{{ address.username }}</span>
        <p @click="changeDefault(address, addressInfo)">
          <span class="s1">{{ address.shipping_address }}</span>
          <span class="s2">{{ address.customer_number }}</span>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userDefaultAddress.shipping_address }}</span>
        收货人：<span>{{ userDefaultAddress.username }}</span>
        <span> {{ userDefaultAddress.customer_number }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.userDefaultAddress.username,
        consigneeTel: this.userDefaultAddress.customer_number,
        deliveryAddress: this.userDefaultAddress.shipping_address,
        paymentWay: "ONLINE",
